...
D_m = {(X_i, Z_i)} for i = 1,...,m,
where X is a d-dimensional real-valued input (X ∈ R^d) and Z ∈ {−1, 1} is a corrupted version of an unobserved true label Y.

The corruption follows an instance-dependent noise model: for a given input x, the observed label is flipped with probability eta(x) ∈ [0, 0.5), and kept unchanged otherwise.
